...
The system includes Symmetric encryption algorithm built-in, so it is a good inspiration for implementing your own algorithm by implementing the [`ISaveSystemEncryption`](xref:Bayat.SaveSystem.Security.ISaveSystemEncryption):
...
For using your own custom encryption algorithm, you should do it through programatic interface by setting the [`SaveSystemSettings.EncryptionAlgorithm`](xref:Bayat.SaveSystem.SaveSystemSettings.EncryptionAlgorithm) property to your own class.